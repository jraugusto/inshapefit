...
(function ($) {
    ...
    var G5_Core_Animation = function($wrapper,delay) {
        if (typeof $wrapper !== 'undefined') {
            $wrapper = $body;
        }
        ...
        initSticky: function ($wrapper) {
            ...
            if (G5_Core.util.isDesktop()) {
                $('.gf-sticky', $wrapper).each(function () {
                    ...
                    $this.off('click').on('click', function () {
                        var sticky = $(this),
                            newHeight = sticky.outerHeight();
                        if(newHeight != oldHeight) {
                            if (sticky.data('hcSticky')) {
                                sticky.hcSticky('destroy');
                            }
                            setTimeout(function () {
                                var config = $.extend({}, defaults, sticky.data("sticky-options"));
                                sticky.hcSticky(config);
                            }, 200);
                        }
                    });
                });
        ...
        sticky: function () {
            ...
            $(window).scroll(function() {
                ...
                if (scrollOffset > currentScrollOffset) {
                    $('.header-sticky').each(function () {
                        if ($(this).find('.menu-one-page').length === 0) {
