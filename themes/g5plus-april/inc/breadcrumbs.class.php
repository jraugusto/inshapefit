...
if (!class_exists('G5Plus_Inc_Breadcrumbs')) {
	class G5Plus_Inc_Breadcrumbs
	{
		...
		public function init()
		{
			...
			$default = array(
				...
				'show_terms' => false,
				'home_label' => esc_html__('Home', 'g5plus-april'),
				'blog_label' => esc_html__('Blog', 'g5plus-april'),
				'tag_archive_prefix' => esc_html__('Tag:', 'g5plus-april'),
				'search_prefix' => esc_html__('Search:', 'g5plus-april'),
				'error_prefix' => esc_html__('Page Not Found', 'g5plus-april')
			);

			$args = apply_filters('g5plus_breadcrumbs_options', $default);
			...
			$this->home_label = isset($default['home_label']) ? $default['home_label'] : esc_html__('Home', 'g5plus-april');
			$this->blog_label = isset($default['blog_label']) ? $default['blog_label'] : esc_html__('Blog', 'g5plus-april');
			$this->tag_archive_prefix = isset($default['tag_archive_prefix']) ? $default['tag_archive_prefix'] : esc_html__('Tag:', 'g5plus-april');
			$this->search_prefix = isset($default['search_prefix']) ? $default['search_prefix'] : esc_html__('Search:', 'g5plus-april');
			$this->error_prefix = isset($default['error_prefix']) ? $default['error_prefix'] : esc_html__('Page Not Found', 'g5plus-april');
		...
		{
			if ($this->html_markup) {
				$this->html_markup = '<ul class="gf-breadcrumbs">' . $this->html_markup . '</ul>';
			}
		...
		private function get_post_terms()
		{
			...
			if ($this->post->post_type === 'post') {
				$taxonomy = 'category';
			} elseif ($this->post->post_type === 'gf-portfolio') {
