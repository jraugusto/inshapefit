            ...
            foreach ($product_images as $key => $value) {
                ...
                if (!empty($variation_id)) { ?>
                    <a href="<?php echo esc_url($image_link); ?>" class="woocommerce-thumbnail-image"
                       title="<?php echo esc_html($image_caption); ?>"
                       data-variation_id="<?php echo esc_attr($variation_id); ?>"
                       data-magnific="true"
                       data-magnific-options='<?php echo json_encode(array('galleryId' => $gallery_id)); ?>'
                       data-index="<?php echo esc_attr($index); ?>" data-large-image="<?php echo esc_url($large_img) ?>"
                       data-gallery-id="<?php echo esc_attr($gallery_id) ?>"><?php echo sprintf('%s', $image); ?></a>
                <?php } else { ?>
                    <a href="<?php echo esc_url($image_link); ?>" class="woocommerce-thumbnail-image"
                       title="<?php echo esc_html($image_caption); ?>" data-index="<?php echo esc_attr($index); ?>"
                       data-magnific="true"
                       data-magnific-options='<?php echo json_encode(array('galleryId' => $gallery_id)); ?>'
                       data-large-image="<?php echo esc_url($large_img) ?>"
                       data-gallery-id="<?php echo esc_attr($gallery_id) ?>"><?php echo sprintf('%s', $image); ?></a>
                <?php }
                echo '</div>';
            }
