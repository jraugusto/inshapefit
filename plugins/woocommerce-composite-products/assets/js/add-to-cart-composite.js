...
( function( $, Backbone ) {
			...
			var Model = Backbone.Model.extend( {
				...
				get_taxed_totals: function( price, regular_price, tax_ratio, qty ) {
					...
					if ( tax_ratio > 0 ) {
						...
						if ( wc_composite_params.prices_include_tax === 'yes' ) {
							totals.price_excl_tax = wc_cp_number_round( totals.price / tax_ratio );
						} else {
							totals.price_incl_tax = wc_cp_number_round( totals.price * tax_ratio );
						}

						if ( wc_composite_params.tax_display_shop === 'incl' ) {
							totals.price = totals.price_incl_tax;
							if ( wc_composite_params.prices_include_tax !== 'yes' ) {
								totals.regular_price = wc_cp_number_round( totals.regular_price * tax_ratio );
							}
						} else {
				...
				calculate_subtotals: function( triggered_by, price_data_array, qty ) {
					...
					if ( false === triggered_by ) {
						...
						price_data[ 'base_price_totals' ] = this.get_taxed_totals( base_price, base_regular_price, base_price_tax_ratio, qty );
					}
					...
					$.each( composite.get_components(), function( index, component ) {
						...
						if ( wc_composite_params.calc_taxes === 'yes' ) {
							...
							if ( product_id > 0 && product_qty > 0 && ( price > 0 || regular_price > 0 ) ) {
								...
								totals = model.get_taxed_totals( price, regular_price, tax_ratio, product_qty );
							}
					...
					if ( this.show_addons_totals ) {
						...
						if ( model.get( 'passes_validation' ) ) {
							...
							var price_data       = $.extend( true, {}, model.price_data ),
								...
								tax_ratio        = Number( price_data.base_price_tax ),
								addons_raw_price = this.$addons_totals.data( 'addons-raw-price' );

							if ( addons_raw_price > 0 ) {

								var addons_prices      = model.get_taxed_totals( addons_raw_price, addons_raw_price, tax_ratio, qty ),
									price_html         = '',
									price_html_suffix  = '',
									addons_totals_html = '';
		...
		this.Composite_Navigation_View = function( opts ) {
			...
			var View = Backbone.View.extend( {
				...
				selection_changed_handler: function( step ) {
					...
					if ( step.is_current() && step.autotransition() && step.passes_validation() && step.get_selected_product() > 0 ) {
					...
					if ( self.show_addons_totals ) {
						...
						if ( self.passes_validation() ) {
							...
							var qty           = self.get_selected_quantity(),
								tax_ratio     = Number( price_data.prices_tax[ self.component_id ] ),
								addons_totals = composite.data_model.get_taxed_totals( addons_price, addons_price, tax_ratio, qty );

							if ( addons_totals.price > 0 ) {

								var price              = Number( price_data.prices[ self.component_id ] ),
									total              = price + Number( addons_price ),
									totals             = composite.data_model.get_taxed_totals( total, total, tax_ratio, qty ),
	...
	wc_cp_classes.WC_CP_Scenarios_Manager = function( composite ) {
		...
		this.init = function() {
			...
			/**
			 * Update the active scenarios when the visibility of a step changes.
			 */
			composite.actions.add_action( 'step_visibility_changed', this.step_visibility_changed_handler, 5, this );
		...
		this.step_visibility_changed_handler = function( triggered_by ) {

			composite.console_log( 'debug:scenarios', '\nUpdating active scenarios in response to "' + triggered_by.get_title() + '" visibility state change...' );
		...
		this.calculate_active_scenarios = function( ref_step, up_to_ref, excl_ref ) {
			...
			$.each( composite.get_components(), function( index, component ) {
				...
				var product_id   = component.get_selected_product( false, true ),
					product_type = component.get_selected_product_type();
	...
	wc_cp_classes.WC_CP_Step_Factory = function() {
		...
		function WC_CP_Component( composite, $component, index ) {
			...
			this.init_qty_input = function() {
				...
				if ( 'yes' === composite.settings.component_qty_restore ) {
