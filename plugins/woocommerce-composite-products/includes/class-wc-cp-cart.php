		...
		if ( is_object( $composite ) && 'composite' === $composite->get_type() ) {
			...
			foreach ( $components as $component_id => $component ) {
				...
				if ( ! in_array( $composited_product_id, $component->get_options() ) ) {
					if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
						wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart. Please choose a valid &quot;%2$s&quot; option&hellip;', 'woocommerce-composite-products' ), $composite_title, $component_title ), 'error' );
					} elseif ( 'cart' === $context ) {
						wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased &ndash; the chosen &quot;%2$s&quot; option is unavailable.', 'woocommerce-composite-products' ), $composite_title, $component_title ), 'error' );
					}
					return false;
				}
				...
				if ( ! $composited_product_wrapper = $composite->get_component_option( $component_id, $composited_product_id ) ) {
					if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
						wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart. Please choose another &quot;%2$s&quot; option&hellip;', 'woocommerce-composite-products' ), $composite_title, $component_title ), 'error' );
					} elseif ( 'cart' === $context ) {
						wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased &ndash; the chosen &quot;%2$s&quot; option is unavailable.', 'woocommerce-composite-products' ), $composite_title, $component_title ), 'error' );
					}
					return false;
				...
				if ( 'variable' === $composited_product_type ) {
					...
					if ( ! $all_set ) {
						if ( $missing_attributes ) {
							...
							if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
    							wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart. %2$s.', 'woocommerce-composite-products' ), $composite_title, $required_fields_notice ), 'error' );
    						} elseif ( 'cart' === $context ) {
								wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased. %2$s.', 'woocommerce-composite-products' ), $composite_title, $required_fields_notice ), 'error' );
    						}
    						return false;
						} else {
							if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
    							wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart. Please choose &quot;%2$s&quot; options&hellip;', 'woocommerce-composite-products' ), $composite_title, $component_title ), 'error' );
    						} elseif ( 'cart' === $context ) {
								wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased. &quot;%2$s&quot; is missing some required options.', 'woocommerce-composite-products' ), $composite_title, $component_title ), 'error' );
    						}
							return false;
						}
					}
				...
				if ( in_array( $error_code, array( 'woocommerce_composite_configuration_selection_required', 'woocommerce_composite_configuration_selection_invalid' ) ) ) {
					...
					if ( ! empty( $error_data[ 'component_id' ] ) ) {
						...
						if ( 'woocommerce_composite_configuration_selection_required' === $error_code ) {
							...
							if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
								wc_add_notice( sprintf( __( 'Please select a &quot;%s&quot; option.', 'woocommerce-composite-products' ), $validation_data[ $error_data[ 'component_id' ] ][ 'title' ] ), 'error' );
							} elseif ( 'cart' === $context ) {
								wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased. A &quot;%2$s&quot; selection is required.', 'woocommerce-composite-products' ), $composite_title, $validation_data[ $error_data[ 'component_id' ] ][ 'title' ] ), 'error' );
							}

						} elseif ( 'woocommerce_composite_configuration_selection_invalid' === $error_code ) {

							if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
								wc_add_notice( sprintf( __( 'Please select a different &quot;%s&quot; option &mdash; the selected product cannot be purchased at the moment.', 'woocommerce-composite-products' ), $validation_data[ $error_data[ 'component_id' ] ][ 'title' ], $validation_data[ $error_data[ 'component_id' ] ][ 'title' ] ), 'error' );
							} elseif ( 'cart' === $context ) {
								wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased &ndash; the chosen &quot;%2$s&quot; option is unavailable.', 'woocommerce-composite-products' ), $composite_title, $validation_data[ $error_data[ 'component_id' ] ][ 'title' ] ), 'error' );
							}
						}
					...
					if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
						...
						wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased. The selected options cannot be purchased together.', 'woocommerce-composite-products' ), $validation_data[ $error_data[ 'component_id' ] ][ 'title' ] ), 'error' );
					}
			...
			foreach ( $validation_data as $component_id => $component_validation_data ) {
				...
				if ( $qty < $qty_min && 'yes' !== $sold_individually ) {
					if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
						wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart. The quantity of &quot;%2$s&quot; cannot be lower than %3$d.', 'woocommerce-composite-products' ), $composite_title, $component_validation_data[ 'title' ], $qty_min ), 'error' );
					} elseif ( 'cart' === $context ) {
						wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased. The quantity of &quot;%2$s&quot; cannot be lower than %3$d.', 'woocommerce-composite-products' ), $composite_title, $component_validation_data[ 'title' ], $qty_min ), 'error' );
					}
					return false;
				} elseif ( $qty_max && $qty > $qty_max ) {
					if ( in_array( $context, array( 'add-to-cart', 'add-to-order' ) ) ) {
						wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart. The quantity of &quot;%2$s&quot; cannot be higher than %3$d.', 'woocommerce-composite-products' ), $composite_title, $component_validation_data[ 'title' ], $qty_max ), 'error' );
					} elseif ( 'cart' === $context ) {
						wc_add_notice( sprintf( __( 'The &quot;%1$s&quot; configuration found in your cart cannot be purchased. The quantity of &quot;%2$s&quot; cannot be higher than %3$d.', 'woocommerce-composite-products' ), $composite_title, $component_validation_data[ 'title' ], $qty_max ), 'error' );
					}
					return false;
				}
			}
		...
		if ( $cart_item[ 'data' ]->needs_shipping() ) {
			...
			if ( $component_option && false === $component_option->is_shipped_individually() ) {
				...
				if ( apply_filters( 'woocommerce_composited_product_has_bundled_weight', false, $cart_item[ 'data' ], $component_id, $composite ) ) {
					$cart_item[ 'data' ]->add_meta_data( '_wc_cp_composited_weight', $cart_item[ 'data' ]->get_weight( 'edit' ), true );
				}
				...
				if ( false === $updating_composite_in_cart && ( $composite = wc_get_product( $product_id ) ) && $composite->is_sold_individually() ) {
					foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
						if ( $product_id === $cart_item[ 'product_id' ] && 'product' === $composite->get_sold_individually_context() ) {
							throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(), __( 'View Cart', 'woocommerce' ), sprintf( __( 'You cannot add another &quot;%s&quot; to your cart.', 'woocommerce' ), $composite->get_title() ) ) );
