...
class WC_CP_Admin_Notices {
	...
	public static function save_notices() {
		...
		delete_option( 'wc_cp_maintenance_notices' );
		add_option( 'wc_cp_meta_box_notices', self::$meta_box_notices );
		add_option( 'wc_cp_maintenance_notices', self::$maintenance_notices );
	}
	...
	public static function output_notices() {
		...
		$saved_notices = maybe_unserialize( get_option( 'wc_cp_meta_box_notices', array() ) );
		...
		if ( ! empty( $notices ) ) {
			...
			foreach ( $notices as $notice ) {
				...
				echo '<div class="wc_cp_notice notice-' . $notice[ 'type' ] . ' notice ' . $dismiss_class . '">';

				if ( $notice[ 'dismiss_class' ] ) {
					$dismiss_url = esc_url( wp_nonce_url( add_query_arg( 'dismiss_wc_cp_notice', $notice[ 'dismiss_class' ] ), 'wc_cp_dismiss_notice_nonce', '_wc_cp_admin_nonce' ) );
					echo '<a class="wc-cp-dismiss-notice notice-dismiss" href="' . $dismiss_url . '">' . __( 'Dismiss', 'woocommerce' ) . '</a>';
				}
	...
	public static function update_notice() {
		...
		if ( WC_CP_Install::is_update_pending() ) {
			...
			if ( WC_CP_Install::is_update_process_running() || WC_CP_Install::is_update_queued() ) {
				...
				$status = __( 'Your database is being updated in the background.', 'woocommerce-composite-products' );
			...
			// Show a prompt to update.
				...
				$status  = __( 'Your database needs to be updated to the latest version.', 'woocommerce-composite-products' );
				$status .= self::get_trigger_update_prompt();

			} elseif ( WC_CP_Install::is_update_incomplete() ) {

				$status  = __( 'Database update incomplete.', 'woocommerce-composite-products' );
				$status .= self::get_failed_update_prompt();
			}

			if ( $status ) {
				$notice = '<strong>' . __( 'WooCommerce Composite Products Data Update', 'woocommerce-composite-products' ) . '</strong> &#8211; ' . $status;
				self::add_notice( $notice, 'info' );
			}

		// Show persistent notice to indicate that the update process is complete.
		} else {
			$notice = __( 'WooCommerce Composite Products data update complete.', 'woocommerce-composite-products' );
	...
	private static function get_force_update_prompt() {
		...
		if ( gmdate( 'U' ) - $update_runtime > 5 ) {
			...
			$fallback_link   = '<a href="' . $fallback_url . '">' . __( 'run the update process manually', 'woocommerce-composite-products' ) . '</a>';
			$fallback_prompt = '<br/><em>' . sprintf( __( '&hellip;Taking a while? You may need to %s.', 'woocommerce-composite-products' ), $fallback_link ) . '</em>';
		}
	...
	private static function get_failed_update_prompt() {
		...
		$support_prompt = '<br/><em>' . sprintf( __( 'If this message persists, please restore your database from a backup, or %s.', 'woocommerce-composite-products' ), $support_link ) . '</em>';
