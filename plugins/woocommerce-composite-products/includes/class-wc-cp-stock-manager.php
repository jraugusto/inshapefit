	...
	public function validate_stock() {
		...
		foreach ( $managed_items as $managed_item_id => $managed_item ) {
			...
			if ( $product_data->is_sold_individually() && $quantity > 1 ) {
				...
				wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart &mdash; only 1 &quot;%2$s&quot; may be purchased.', 'woocommerce-composite-products' ), $composite_title, $product_data->get_title() ), 'error' );
				return false;
			}

			// Check product is_purchasable.
			if ( ! $product_data->is_purchasable() ) {
				wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart &mdash; &quot;%2$s&quot; cannot be purchased.', 'woocommerce-composite-products' ), $composite_title, $product_data->get_title() ), 'error' );
				return false;
			}
			...
			if ( 'variation' === $product_data->get_type() ) {
				$configuration = sprintf( _x( ' (%s)', 'suffix', 'woocommerce-composite-products' ), wc_get_formatted_variation( $product_data, true ) );
			}
			...
			if ( ! $product_data->is_in_stock() ) {
				...
				if ( $is_variable ) {
					wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart &mdash; the chosen &quot;%2$s&quot; variation%3$s is out of stock.', 'woocommerce-composite-products' ), $composite_title, $product_data->get_title(), $configuration ), 'error' );
				} else {
					wc_add_notice( sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart &mdash; &quot;%2$s&quot; is out of stock.', 'woocommerce-composite-products' ), $composite_title, $product_data->get_title() ), 'error' );
				}
				...
				if ( $is_variable ) {
					wc_add_notice( sprintf(__( 'This &quot;%1$s&quot; configuration cannot be added to the cart &mdash; the chosen &quot;%2$s&quot; variation%3$s does not have enough stock (%4$s remaining).', 'woocommerce-composite-products' ), $composite_title, $product_data->get_title(), $configuration, $product_data->get_stock_quantity() ), 'error' );
				} else {
					wc_add_notice( sprintf(__( 'This &quot;%1$s&quot; configuration cannot be added to the cart &mdash; there is not enough stock of &quot;%2$s&quot; (%3$s remaining).', 'woocommerce-composite-products' ), $composite_title, $product_data->get_title(), $product_data->get_stock_quantity() ), 'error' );
				}
				...
				// Products.
					...
					if ( isset( $quantities_in_cart[ $managed_item_id ] ) && ! $product_data->has_enough_stock( $quantities_in_cart[ $managed_item_id ] + $quantity ) ) {
						wc_add_notice( sprintf(
							...
							WC()->cart->get_cart_url(),
							__( 'View Cart', 'woocommerce' ),
							sprintf( __( 'This &quot;%1$s&quot; configuration cannot be added to the cart because there is not enough stock of &quot;%2$s&quot; &mdash; we have %3$s in stock and you already have %4$s in your cart.', 'woocommerce-composite-products' ), $composite_title, $product_data->get_title(), $product_data->get_stock_quantity(), $quantities_in_cart[ $managed_item_id ] )
						), 'error' );
