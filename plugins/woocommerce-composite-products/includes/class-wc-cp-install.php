...
class WC_CP_Install {
	...
	private static function must_install() {
		return self::$current_version !== WC_CP()->version;
	}
	...
	private static function can_install() {
		return false === get_transient( 'wc_cp_installing' ) && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) && ! defined( 'IFRAME_REQUEST' ) && current_user_can( 'manage_woocommerce' );
	}
	...
	private static function can_update() {
		return self::can_install() && self::$current_version === WC_CP()->version;
	}
	...
	public static function plugin_row_meta( $links, $file ) {
		...
		if ( $file == WC_CP()->plugin_basename() ) {
			$row_meta = array(
				'docs'    => '<a href="https://docs.woocommerce.com/document/composite-products/">' . __( 'Documentation', 'woocommerce-composite-products' ) . '</a>',
				'support' => '<a href="' . esc_url( WC_CP_SUPPORT_URL ) . '">' . __( 'Support', 'woocommerce-composite-products' ) . '</a>',
			);
