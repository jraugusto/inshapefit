	...
	*/
	...
	public static function get_product_price( $product, $args ) {
		...
		if ( $price ) {
			...
			if ( 'incl_tax' === $calc && 'yes' === wc_cp_calc_taxes() && 'yes' !== wc_cp_prices_include_tax() ) {
				$price = wc_get_price_including_tax( $product, array( 'qty' => $qty, 'price' => $price ) );
			} elseif ( 'excl_tax' === $calc && 'yes' === wc_cp_calc_taxes() && 'yes' === wc_cp_prices_include_tax() ) {
	...
	*/
	...
	public static function filter_available_variation( $variation_data, $product, $variation ) {
		...
		if ( ! empty( $filtered_component_option  ) ) {
			...
			$price_incl_tax                        = wc_get_price_including_tax( $variation, array( 'qty' => 1, 'price' => 1000 ) );
			$price_excl_tax                        = wc_get_price_excluding_tax( $variation, array( 'qty' => 1, 'price' => 1000 ) );
