...
class WC_CP_PIP_Compatibility {
	...
	public static function remove_filters() {
		remove_filter( 'woocommerce_order_get_items', array( WC_CP()->order, 'get_order_items' ), 10 );
		remove_filter( 'woocommerce_order_item_product', array( WC_CP()->order, 'get_product_from_item' ), 10 );
	}
		...
		if ( self::$document && 'pick-list' === self::$document->type ) {
			...
			if ( wc_cp_is_composited_order_item( $item ) ) {
				...
				if ( 'no' === $item->get_meta( '_composite_item_needs_shipping', true ) ) {
					...
					$flat        = false;
					$parent_item = wc_cp_get_composited_order_item_container( $item );
	 ...
	 * @return boolean
	 */
	public static function hide_item( $hide, $product, $item, $order ) {

		if ( self::$recounting_items && wc_cp_is_composite_container_order_item( $item ) ) {
			$hide = false;
		} elseif ( wc_cp_is_composited_order_item( $item, $order ) ) {
			if ( self::$document && 'packing-list' === self::$document->type && 'no' === $item->get_meta( '_composite_item_needs_shipping', true ) ) {
				$hide = true;
			}
		}
