	...
	class WC_Product_Gift_Card extends WC_Product {
		...
		public function get_product_amounts() {
			...
			if ( ! is_array ( $this->amounts ) ) {
				if ( $this->id ) {
					$result        = yit_get_prop ( $this->get_product_instance (), self::YWGC_AMOUNTS, 'edit' );
		...
		public function add_to_cart_text() {
			$text = $this->is_purchasable () && $this->is_in_stock () ? __ ( 'Select amount', 'yith-woocommerce-gift-cards' ) : __ ( 'Read more', 'yith-woocommerce-gift-cards' );
