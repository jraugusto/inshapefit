...
if ( ! class_exists ( 'YITH_WooCommerce_Gift_Cards_Premium' ) ) {
	...
	class YITH_WooCommerce_Gift_Cards_Premium extends YITH_WooCommerce_Gift_Cards {
		...
		/**
		 * @var bool enable internal notes on admin side
		 */
		public $enable_internal_notes = false;
		...
		public function init_hooks() {
			...
			add_filter ( 'yith_ywgc_gift_card_instance_metabox_custom_fields', array(
				$this,
				'add_internal_note_field'
			) );
		...
		public function init_plugin() {
			...
			$this->enable_internal_notes     = "yes" == get_option ( 'ywgc_enable_internal_notes', 'no' );
		...
		public function add_internal_note_field( $fields ) {
			if ( $this->enable_internal_notes ) {

				$fields['_ywgc_internal_notes'] = array(
...
//				'show_in_menu'        => true,
		...
		public function preview_digital_gift_cards( $object, $context = 'shop' ) {
			...
			if ( $object instanceof WC_Product ) {
				...
				$amount = 0;
				if ( $object instanceof WC_Product_Simple || $object instanceof WC_Product_Variable ) {
