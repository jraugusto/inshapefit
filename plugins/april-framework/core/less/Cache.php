...
class Less_Cache{
	...
	public static function Get( $less_files, $parser_options = array(), $modify_vars = array() ){
		...
		if( !empty($modify_vars) ){
			...
			if( !file_exists($vars_file) ){
				G5P()->file()->putContents($vars_file, $lessvars);
			}
		...
		$list[] = $compiled_name;
		$cache = implode("\n",$list);
        G5P()->file()->putContents( $list_file, $cache);


		//save the css
        G5P()->file()->putContents( $output_file, $compiled);
	...
	public static function Cache( &$less_files, $parser_options = array() ){
		...
		$file = GF_PLUGIN_DIR . 'core/less/Less.php';
	...
	public static function CheckCacheDir(){
		...
		if( !file_exists(Less_Cache::$cache_dir) ){
			if( !G5P()->file()->mkdir(Less_Cache::$cache_dir) ){
	...
	static function ListFiles($list_file, &$list, &$css_file_name ){
		...
		$list = explode("\n",G5P()->file()->getContents($list_file));
