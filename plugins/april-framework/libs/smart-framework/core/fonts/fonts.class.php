...
if (!class_exists('GSF_Core_Fonts')) {
    class GSF_Core_Fonts
    {
        ...
        {
            $fontManagementEnable =  apply_filters('gsf_font_management_enable',false);
            if (!$fontManagementEnable) return;
        ...
        public function menu()
        {
            add_submenu_page(
                'gsf_welcome',
                esc_html__('Fonts Management', 'april-framework'),
                esc_html__('Fonts Management', 'april-framework'),
                'manage_options',
                '_fonts_management',
            ...
            if ($current_page == '_fonts_management') {
        ...
        public function adminEnqueueScripts()
        {
            ...
            wp_localize_script(GSF()->assetsHandle('fonts'), 'GSF_META_DATA', array(
                ...
                'msgConfirmDeleteCustomFont' => esc_html__('Are you sure delete custom font?','april-framework'),
                'msgConfirmRemoveActiveFont' => esc_html__('Are you sure remove this font?','april-framework'),
                'msgConfirmResetFont' => esc_html__('Are you sure reset active fonts?','april-framework'),
            ));
        }
        ...
        {
            return array(
                'google'   => esc_html__('Google Fonts', 'april-framework'),
                'standard' => esc_html__('Standard Fonts', 'april-framework'),
                'custom'   => esc_html__('Custom Fonts', 'april-framework'),
            );
        }
        ...
        private function &getGoogleFontsSource() {
            $fonts = json_decode(G5P()->file()->getContents(GSF()->pluginDir('assets/webfonts.json')), true);
            return $fonts;
        }
        ...
        public function ajaxUploadFonts() {
            $nonce = $_REQUEST['_nonce'];
            if (!wp_verify_nonce($nonce, GSF()->helper()->getNonceVerifyKey())) {
                wp_send_json_error(esc_html__('Access deny!','april-framework'));
                die();
            }
            $name = $_POST['name'];
            ...
            if (!file_exists($this->fontResoucesDir())) {
                G5P()->file()->mkdir($this->fontResoucesDir());
            }
                ...
                if ($css_count === 1) {
                    if ($create_font_dir) {
                        if (@file_exists($this->fontResoucesDir() . $sanitize_name)) {
                            wp_send_json_error(esc_html__('Font Exist!','april-framework'));
                            $zip->close();
                            unlink($target_file);
                            die();
                        }
                        G5P()->file()->mkdir($this->fontResoucesDir() . $sanitize_name);
            ...
            if ($css_count === 1) {
                ...
                $css_content = G5P()->file()->getContents($this->fontResoucesDir() . $css_file);

                $count = preg_match_all('/@font-face\s*\{([^\{]*(?=\}))\}/', $css_content, $matches);
                if (!$count) {
                    wp_send_json_error(esc_html__('Missing css file!','april-framework'));
                    die();
                }
                ...
                wp_send_json_error(esc_html__('Only allowed with 1 css file!','april-framework'));
            }
            die();
        }
        ...
        public function ajaxDeleteCustomFont() {
            ...
            foreach ($option_fonts as $key => $font) {
                if ($font['family'] == $family_name) {
                    if (G5P()->file()->delete($this->fontResoucesDir() . $font['font_dir'], true, 'd')) {
                    ...
                    wp_send_json_error(esc_html__('Font directory is not deleted','april-framework'));
                    die();
                }
            }
            wp_send_json_success(esc_html__('Delete custom font done!','april-framework'));
            die();
        }
        ...
        public function ajaxUsingFont() {
            ...
            foreach ($options as $font) {
                if ($font['family'] == $font_data['family']) {
                    wp_send_json_error(esc_html__('Fonts already exists!','april-framework'));
                    die();
                }
            }
        ...
        public function ajaxSaveActiveFont() {
            ...
            $this->updateActiveFonts($options);
            wp_send_json_success(esc_html__('Save Done!','april-framework'));
            die();
        }
    }
}