...
if (!class_exists('GSF_Inc_Admin_Ajax')) {
	class GSF_Inc_Admin_Ajax {
		...
		{
			$webfonts = json_decode(G5P()->file()->getContents(GSF()->pluginDir('assets/webfonts.json')), true);
			...
			$fonts = array(
				'basic'  => array(
					'label' => esc_html__('Standard Fonts', 'april-framework'),
				),
				'google' => array(
					'label' => esc_html__('Google Webfonts', 'april-framework'),
				),
			);
