 ...
 */
$el_class = $map_height = $api_key = $map_zoom = $scroll_wheel = $map_style = $map_style_content = $markers =
...
if(intval($map_zoom) <=0 || intval($map_zoom) > 18 ) {
    $map_zoom = 15;
}
...
if ($map_height != '') {
    $wrapper_styles[] = "height: {$map_height}";
}
if ($wrapper_styles) {
...
wp_enqueue_script('google-map', $protocol . '//maps.googleapis.com/maps/api/js?libraries=places&language=' . get_locale() . '&key=' . esc_html($googlemap_api_key), array('jquery'), '1.0', false);
$map_id = 'map-' . uniqid();
?>
    ...
    (function ($) {
        ...
        var G5PlusGoogleMap = {
            ...
            settingMap: function () {
                ...
                break;
                case 'custom':?>
                mapStyle = <?php echo(rawurldecode(base64_decode(strip_tags($map_style_content)))); ?>;
                <?php
                break;
                default:?>
                mapStyle = [];
                ...
                var markers = [
                    ...
                    foreach ($markers as $data) {
                        ...
                        $lat = isset($data['lat']) ? $data['lat'] : '18.6725822';
                        $lng = isset($data['lng']) ? $data['lng'] : '105.6744326';
